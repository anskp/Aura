...
var configSchema = exports_external.object({
  ...
  oracleConsumerAddress: exports_external.string(),
  poolId: exports_external.string(),
  assetId: exports_external.string(),
  gasLimit: exports_external.string()
});
var payloadSchema = exports_external.object({
  nav: exports_external.string(),
  reserve: exports_external.string(),
  timestamp: exports_external.number().optional(),
  poolId: exports_external.string().optional(),
...
var onHTTPTrigger = (runtime2, payload) => {
  ...
  const poolId = parsed.poolId ?? runtime2.config.poolId;
  const assetId = parsed.assetId ?? runtime2.config.assetId;
  const nav = BigInt(parsed.nav);
  const reserve = BigInt(parsed.reserve);
  const timestamp = parsed.timestamp ?? Math.floor(Date.now() / 1000);
  const reportId = `0x${Buffer.from(`navpor:${timestamp}`).toString("hex").padEnd(64, "0").slice(0, 64)}`;
  ...
  const resp = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.oracleConsumerAddress,
